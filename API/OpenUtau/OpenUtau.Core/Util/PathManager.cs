...
namespace OpenUtau.Core {
    ...
    public class PathManager : SingletonBase<PathManager> {
        public PathManager() {
            Console.WriteLine("RootPath WIPED");
            ...
            if (OS.IsMacOS()) {
                ...
                // Use the OpenUtau.Core assembly location so that when loaded via
                // pythonnet the path resolves to the UtauGenerate bin directory
                // (where Singers/ is copied by the csproj), not the Python exe.
                string asmPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                DataPath = asmPath;
                Console.WriteLine("Using local data folder: " + DataPath);
