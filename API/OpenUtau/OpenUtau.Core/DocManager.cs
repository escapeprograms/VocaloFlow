...
namespace OpenUtau.Core {
    ...
    public class DocManager : SingletonBase<DocManager> {
        ...
        public void Initialize(Thread mainThread, TaskScheduler mainScheduler) {
            ...
            // foreach (var factory in PhonemizerFactories) {
            //     Console.WriteLine($"Found phonemizer {factory.type.FullName}");
            // }
        }
        ...
        public void SearchAllPlugins() {
            ...
            try {
                ...
                Console.WriteLine("search dir: " + Path.Combine(asmDir, kBuiltin));
                files.Add(Path.Combine(asmDir, kBuiltin));
                ...
                Log.Error(e, "Failed to search plugins.");//TODO
                                                          //try again under a different directory
                String backupPath = Path.Combine(PathManager.Inst.DataPath, "assets");
                Console.WriteLine("backup search dir: " + Path.Combine(backupPath, kBuiltin));
                files.Add(Path.Combine(backupPath, kBuiltin));
                Directory.CreateDirectory(PathManager.Inst.PluginsPath);
                string oldBuiltin = Path.Combine(PathManager.Inst.PluginsPath, kBuiltin);
                if (File.Exists(oldBuiltin)) {
                    File.Delete(oldBuiltin);
