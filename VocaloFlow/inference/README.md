...
- **CFG (`cfg_scale > 1.0`):** Each velocity query runs both a conditional pass and an unconditional pass with `f0`, `voicing`, and `phoneme_ids` zeroed out, then returns `v_uncond + cfg_scale * (v_cond - v_uncond)`. Doubles cost when enabled.
