    ...
    def __init__(self, model_path: str, device: str):
        try:
            ...
            if 'torch.distributed.device_mesh' not in sys.modules:
                sys.modules['torch.distributed.device_mesh'] = types.ModuleType('device_mesh')
            ...
            try:
                ...
                torch.distributed.tensor.parallel = types.ModuleType('parallel')
                sys.modules['torch.distributed.tensor'] = torch.distributed.tensor
                sys.modules['torch.distributed.tensor.parallel'] = torch.distributed.tensor.parallel

            if not hasattr(torch.distributed.tensor.parallel, 'SequenceParallel'):
                torch.distributed.tensor.parallel.SequenceParallel = type('SequenceParallel', (object,), {})
                
            import nemo.collections.asr as nemo_asr  # type: ignore
        except Exception as e:  # pragma: no cover
            import traceback
            traceback.print_exc()
            print(f"Underlying NeMo import error: {e}")
            raise ImportError(
        ...
        if hasattr(self.model, 'cfg') and hasattr(self.model.cfg, 'decoding'):
            with open_dict(self.model.cfg.decoding):
                if 'greedy' in self.model.cfg.decoding:
