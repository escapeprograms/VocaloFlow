...
def process(args, config, model: torch.nn.Module):
    ...
    if args.save_mel:
        # Calculate total mel frames: generated_len / hop_size
        total_mel_frames = int(np.ceil(generated_len / config.audio.hop_size))
        # Get n_mels from config
        n_mels = config.model.flow_matching.mel_dim
        generated_merged_mel = np.zeros((total_mel_frames, n_mels), dtype=np.float32)
    ...
    for idx, target_meta in enumerate(
        ...
        with torch.no_grad():
            if args.save_mel:
                generated_audio, generated_mel = model.infer(
                    infer_data,
                ...
                # generated_mel shape: [1, T, D]
                mel_np = generated_mel.squeeze(0).cpu().numpy()
                start_frame = int(start_sample_idx / config.audio.hop_size)
                end_frame = start_frame + mel_np.shape[0]
                # Truncate if the segment mel overflows the pre-allocated buffer
                actual_end = min(end_frame, generated_merged_mel.shape[0])
                frames_to_copy = actual_end - start_frame
                if frames_to_copy > 0:
                    generated_merged_mel[start_frame:actual_end, :] = mel_np[:frames_to_copy]
            else:
        ...
        # Truncate if the segment audio overflows the pre-allocated buffer
        end_sample = start_sample_idx + generated_audio.shape[0]
        actual_end = min(end_sample, generated_len)
        samples_to_copy = actual_end - start_sample_idx
        if samples_to_copy > 0:
            generated_merged[start_sample_idx:actual_end] = generated_audio[:samples_to_copy]

    merged_path = os.path.join(args.save_dir, "target.wav")
    sf.write(merged_path, generated_merged, config.audio.sample_rate)
    print(f"Generated audio saved to {merged_path}")

    if args.save_mel:
        mel_path = os.path.join(args.save_dir, "target_mel.npy")
        np.save(mel_path, generated_merged_mel)
        print(f"Generated mel-spectrogram saved to {mel_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_mel", action="store_true", help="Save the produced mel-spectrogram as .npy")
    parser.add_argument(
        "--control",
        type=str,
        default="melody",
