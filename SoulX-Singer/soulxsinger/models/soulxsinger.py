...
class SoulXSinger(nn.Module):
    ...
    def infer(self, meta: dict, auto_shift=False, pitch_shift=0, n_steps=32, cfg=3, control="melody", return_mel=False):
        ...
        # generated_mel shape: [B, T, D]
        # Vocoder expects: [B, D, T]
        mel_for_vocoder = generated_mel.transpose(1, 2)
        generated_audio = self.vocoder(mel_for_vocoder[0:1, ...])
        
        if return_mel:
            return generated_audio, generated_mel
