...
This directory contains **subprocess entry points** — scripts that are launched as separate processes (in the dedicated `soulxsinger` conda environment) to run GPU-intensive model inference. They are never imported directly; instead they are invoked via `subprocess.run()` from pipeline or stage scripts running in the parent `vocaloflow-datasynthesizer` environment. The interpreter path for the subprocess env comes from `config.SOULX_PYTHON`.
...
- Must be run under the `soulxsinger` conda environment (the subprocess env with torch + SoulX-Singer package), launched via `SOULX_PYTHON`.
